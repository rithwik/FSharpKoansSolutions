...
module ``about the stock example`` =
    ...
    let YouGotTheAnswerCorrect() =
        let result =  stockData
                        ...
                        |> List.tail
                        |> List.map (fun (dt, o, c) -> dt, abs(System.Double.Parse o - System.Double.Parse c))
                        |> List.maxBy (fun (dt, diff) -> diff)
...
// let column columnname data =
//     data
//     |> List.head
//     |> Array.findIndex (fun x -> x = columnname) 

// //greatest variance between the opening and closing price
// let GreatestVariance rawData =
//     let data = rawData |> List.map splitCommas
